Func submitJob()
	...
	For $i = $startingItem to $totalCount - 1
		If $isWorking = 1 Then
			...
			If $currentJobSourcefile <> "ERROR: File for this event not found" And $currentJobSourcefile <> " <source file recycled>" Then ; the file is valid
			...
			Else
				_GUICtrlListView_SetItem($eventsList, "Skipped (no source file)", $i, 6)
			EndIf
